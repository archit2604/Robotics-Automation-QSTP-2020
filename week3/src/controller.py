...
def callback_odom(msg):
    twist=Twist()
    euler = euler_from_quaternion(quaternion=(msg.pose.pose.orientation.x, msg.pose.pose.orientation.y, msg.pose.pose.orientation.z, msg.pose.pose.orientation.w))
    global count
    global diff_sum
    global diff_prev
    global diff
    ...
    if count==1:
        ...
        e_sum+=e
        dedt=e-e_prev
        twist.linear.x=1.5*(e)+0.00044*(e_sum)+28*dedt
        twist.angular.z=0
        pub.publish(twist)
        e_prev=e
    if e<=0.15 and e>=-0.15 and count==1:
        twist.linear.x=0
        twist.angular.z=0
        pub.publish(twist)
        count=2
