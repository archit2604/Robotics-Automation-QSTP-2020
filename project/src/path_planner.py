...
def control():
    ...
    while chk=='y'or chk=='Y':
        x=int(raw_input("Enter the x-coordinate of goal point(0-6):"))
        y=int(raw_input("Enter the y-coordinate of goal point(0-6):"))
        if x>6 or x<0 or y>6 or y<0:
            print("Point outside range.Try Again!!")
            continue
        end=[x,y]
        if chk_in_obs(end):
            print("Goal too close to obstacle")
            continue
        else:
            print("Planning Path")
            path=Path()
        ...
        a=optimize()
        print("Path planning complete.Running the Bot")
        reverse()
        # prm.visualize_path(path_new.poses)
        while not ((end[0]-curr_x<0.1 and end[0]-curr_x>-0.1) and (end[1]-curr_y<0.1 and end[1]-curr_y>-0.1)):
            pub.publish(path_new)
            rospy.sleep(0.1)
        print("Goal Reached")
        beg=end
        chk=raw_input("Do you want to continue?(y/n):")
...
def optimize():
    ...
    if not chk_obs_coor(beg[0],beg[1], path.poses[0].pose.position.x,path.poses[0].pose.position.y):
        return 1
    ...
    while current_index < len(path.poses) - 1:
        ...
        for lookahead_index in range(len(path.poses) - 1 , current_index, -1):
            if not chk_obs_coor(path.poses[current_index].pose.position.x,path.poses[current_index].pose.position.y, path.poses[lookahead_index].pose.position.x,path.poses[lookahead_index].pose.position.y):
...
def chk_obs_coor(x1,y1,x2,y2):
    ...
    for obstacle in obstacle_list:
        if LineString([(x1,y1),(x2,y2)]).intersects(Point(obstacle[0],obstacle[1]).buffer(0.25+0.31+0.13))==True:
            return True
    return False
def chk_obs(i,j):
    global nodes
    global obstacle_list
    for obstacle in obstacle_list:
        if LineString([(nodes[i][0],nodes[i][1]),(nodes[j][0],nodes[j][1])]).intersects(Point(obstacle[0],obstacle[1]).buffer(0.25+0.31+0.13))==True:
            return True
    return False
...
def sampler(sample_area):
    return (random.uniform(sample_area[0], sample_area[1]),random.uniform(sample_area[0], sample_area[1]))
...
class PRM():
    ...
    def visualize_tree(graph, obstacle_list):
        ...
        for node in graph.graph_dict:
            for neighbour in graph.graph_dict[node]:
                    plt.plot([nodes[int(node)][0], nodes[neighbour][0]],[nodes[int(node)][1], nodes[neighbour][1]])
    ...
    def plan(self,beg,end):
        ...
        min_dist=float('inf')
        for node in nodes:
            dist=float(math.sqrt((node[0]-end[0])**2+(node[1]-end[1])**2))
            if dist<min_dist:
                min_dist=dist
                min_index=nodes.index(node)
        ...
        while len(open_)>0:
            ...
            for key in neighbors:
                ...
                neighbor.h = math.sqrt((nodes[int(goal)][0]-nodes[int(neighbor.name)][0])**2+(nodes[int(goal)][1]-nodes[int(neighbor.name)][1])**2)
    ...
    def visualize_path(p):
        ...
        for i in range(len(p)-1):
            plt.plot([p[i].pose.position.x, p[i+1].pose.position.x],[p[i].pose.position.y, p[i+1].pose.position.y])
